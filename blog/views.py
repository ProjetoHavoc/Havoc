...
class PostBusca(PostIndex):
    ...
    def get_queryset(self):
        ...
        if not termo: # se a busca retornar vazia
            return qs # retorna qs todos os resultados sem filtro
...
class PostDetalhes(UpdateView):
    ...
    form_class = FormComentario # indicando qual o form

    context_object_name = 'post' #indicando um nome de contexto para ser usado no template com o for

    def get_context_data(self, **kwargs):#redefinindo o metodo e criando um dicionario

        contexto = super().get_context_data(**kwargs) # fazendo a  busca e armazenando na variavel contexto

        post = self.get_object()

        comentarios = Comentario.objects.filter(publicado_comentario=True, # se nao for checado com o check box como true nao fica na lista da query
